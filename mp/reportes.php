...
async function generarReporte() {
    ...
    document.getElementById('rep-ing-cant').textContent  = (t.cant_ing || 0) + ' movimientos';
    document.getElementById('rep-gasto-cant').textContent= (t.cant_gasto || 0) + ' movimientos';

    const porCat   = data.por_categoria;
    const totalAbs = porCat.reduce((s,r) => s + parseFloat(r.total), 0);

    document.getElementById('rep-tbody').innerHTML = porCat.map(r => {
    ...
    const paletteBlue  = ['#009ee3','#00b1ea','#0ea5e9','#38bdf8','#7dd3fc','#2563eb','#3b82f6','#60a5fa','#93c5fd','#bfdbfe'];

    if (chartGastos) chartGastos.destroy();
    chartGastos = new Chart(document.getElementById('chart-gastos'), {
        type: 'doughnut',
        data: { labels: gastosCats.map(r=>r.cat), datasets: [{ data: gastosCats.map(r=>parseFloat(r.total)), backgroundColor: palette, borderColor:'#0e1320', borderWidth:2 }] },
    ...
    chartIngresos = new Chart(document.getElementById('chart-ingresos'), {
        type: 'doughnut',
        data: { labels: ingresosCats.map(r=>r.cat), datasets: [{ data: ingresosCats.map(r=>parseFloat(r.total)), backgroundColor: paletteBlue, borderColor:'#0e1320', borderWidth:2 }] },
        options: { responsive: true, plugins: {
            legend: { position:'bottom', labels:{ color:'#7a90b0', font:{family:'Syne',size:11}, padding:10 } },
            tooltip: { callbacks: { label: ctx => ' $ ' + ctx.parsed.toLocaleString('es-AR', {minimumFractionDigits:2}) } }
        }}
