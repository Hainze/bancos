...
/**
 * mp/api/padrones.php
 * Consultas MP-específicas (mp_movimientos, mp_lotes)
...
try {
    switch ($action) {
        ...
        case 'listar_movimientos':
            ...
            $stTot=$pdo->prepare("SELECT COUNT(*),
                SUM(CASE WHEN tipo='ingreso' THEN importe ELSE 0 END),
                SUM(CASE WHEN tipo='gasto' THEN ABS(importe) ELSE 0 END)
                FROM mp_movimientos $where");
            $stTot->execute($params);
            [$total,$totalIng,$totalGasto]=$stTot->fetch(PDO::FETCH_NUM);

            $st=$pdo->prepare("SELECT * FROM mp_movimientos $where ORDER BY fecha DESC,id DESC LIMIT $limit OFFSET $offset");
            $st->execute($params);
            echo json_encode([
                'data'           => $st->fetchAll(),
                'total'          => (int)$total,
        ...
        case 'stats_dashboard':
            ...
            $totales=$pdo->prepare("SELECT
                ...
                FROM mp_movimientos WHERE fecha BETWEEN ? AND ?");
            ...
            $lotes=$pdo->query("SELECT * FROM mp_lotes ORDER BY created_at DESC LIMIT 10")->fetchAll();
        ...
        case 'resetear_movimientos':
            $pdo->exec("DELETE FROM mp_movimientos");
            $pdo->exec("DELETE FROM mp_lotes");
            echo json_encode(['success'=>true]);
            break;
