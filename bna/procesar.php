...
<div class="grid-2" style="gap:24px">
    <div>
        <div class="card mb-24">
            <div class="card-title">Subir Archivo — Banco Nacion</div>

            <div class="alert alert-info">
                <span>ℹ</span>
                <div>
                    El módulo detecta automáticamente el formato del extracto BNA:<br>
                    <strong>Formato A (HomeBank):</strong> Fecha · Descripción · Debe · Haber<br>
                    <strong>Formato B (simple):</strong> Fecha · Descripción · Importe (firmado)<br>
            ...
            <div class="upload-zone" id="upload-zone">
                ...
                <div class="upload-title">Arrastrar el Excel del BNA aquí</div>
            ...
            <div id="formato-detectado" style="display:none;margin-top:8px;font-size:12px;color:var(--sub)"></div>
            ...
            <div id="preview-table" style="display:none">
                ...
                <div class="table-wrap" style="max-height:500px;overflow-y:auto">
                    <table>
                        <thead>
                            <tr>
                                <th>Fecha</th><th>Descripción</th><th>Importe</th>
                                <th>Categoría</th><th>Código</th><th>CUIT/DNI</th><th>Nombre</th>
                            </tr>
                        </thead>
...
document.getElementById('btn-limpiar').addEventListener('click', () => {
    currentFile = null; processedData = null;
...
document.getElementById('btn-procesar').addEventListener('click', async () => {
    ...
    try {
        const res  = await fetch('/bna/api/procesar_excel.php', { method: 'POST', body: form });
        ...
        if (data.formato) {
            document.getElementById('formato-resultado').textContent = '✓ Formato detectado: ' + data.formato;
        }

        renderPreview(data.rows);
        toast(`✓ ${data.total} filas procesadas (${data.formato || 'auto'})`, 'success');
    } catch(e) {
...
function renderPreview(rows) {
    ...
    document.getElementById('prev-gasto').textContent = gastos + ' gastos';
    document.getElementById('preview-body').innerHTML = rows.map(r => `
        <tr>
            <td class="mono">${r.fecha||'—'}</td>
            <td style="max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${r.descripcion}">${r.descripcion}</td>
            ...
            <td class="mono" style="font-size:11px">${r.cuit||r.dni||'—'}</td>
            <td style="font-size:12px;max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.nombre||'—'}</td>
        </tr>`).join('');
...
document.getElementById('btn-descargar').addEventListener('click', () => {
    ...
    window.location.href = `/bna/api/exportar_excel.php?tipo=movimientos&lote=${encodeURIComponent(processedData.lote)}`;
});
</script>
