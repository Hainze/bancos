...
<div class="filter-bar">
    ...
    <div class="filter-bar-body">
        ...
        <div class="filter-actions">
            ...
            <a href="/bna/procesar.php" class="btn btn-success">⬆ Importar</a>
...
<div class="card">
    <div class="flex-between mb-16">
        ...
        <a href="/bna/movimientos.php" class="btn btn-secondary btn-sm">≡ Ver todos</a>
    </div>
    <div class="table-wrap">
        <table>
            <thead>
                <tr><th>Código</th><th>Archivo</th><th>Total</th><th>Clasificadas</th><th>Sin clasificar</th><th>Fecha</th><th></th></tr>
            </thead>
            <tbody id="lotes-body">
...
<div class="modal-overlay" id="modal-limpiar">
    <div class="modal" style="max-width:420px">
        ...
        <p style="color:var(--sub);font-size:14px;margin-bottom:16px">
            Esto va a eliminar <strong style="color:var(--text)">todos los movimientos y lotes del Banco Nacion</strong>.
        </p>
        <div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.25);border-radius:8px;padding:12px 16px;margin-bottom:24px;font-size:13px;color:var(--sub)">
            Las <strong style="color:var(--text)">categorías, palabras clave y clientes no se tocan</strong>.<br>
...
function getMonthRange(range) {
    ...
    const pad=n=>String(n).padStart(2,'0'); const curr=`${y}-${pad(m)}`;
    switch(range){
...
async function loadDashboard(){
    try{
        ...
        const ing=parseFloat(t.total_ing||0),gas=parseFloat(t.total_gasto||0),net=ing-gas;
        ...
        waitForChart(()=>{renderCharts(data.por_categoria);});
        renderLotes(data.lotes_recientes);
    }catch(e){toast('Error al cargar el dashboard','error');}
}
...
function renderCharts(porCat){
    const catMap={};
    porCat.forEach(r=>{if(!catMap[r.cat])catMap[r.cat]={ing:0,gasto:0};if(r.tipo==='ingreso')catMap[r.cat].ing+=parseFloat(r.total);else catMap[r.cat].gasto+=parseFloat(r.total);});
    ...
    if(hasData){
        ...
        chartCat=new Chart(document.getElementById('chart-categorias'),{type:'bar',data:{labels:cats,datasets:[
            ...
            {label:'Gastos',data:cats.map(c=>catMap[c].gasto),backgroundColor:'rgba(239,68,68,0.75)',borderColor:'#ef4444',borderWidth:1,borderRadius:4},
        ]},options:{responsive:true,plugins:{legend:{labels:{color:'#7a90b0',font:{family:'Syne',size:11},padding:12}},tooltip:{callbacks:{label:ctx=>` ${ctx.dataset.label}: ${formatMoney(ctx.parsed.y)}`}}},scales:{x:{ticks:{color:'#4a5f7a',maxRotation:35,font:{size:10}},grid:{color:'#1e2d45'}},y:{ticks:{color:'#4a5f7a',callback:v=>'$ '+Intl.NumberFormat('es-AR',{notation:'compact'}).format(v)},grid:{color:'#1e2d45'}}}}});
    }
    ...
    if(hasTorta){
        ...
        chartTorta=new Chart(document.getElementById('chart-torta'),{type:'doughnut',data:{labels:cats,datasets:[{data:tortaData,backgroundColor:palette,borderColor:'#0e1320',borderWidth:2,hoverOffset:8}]},options:{responsive:true,cutout:'62%',plugins:{legend:{position:'bottom',labels:{color:'#7a90b0',font:{family:'Syne',size:11},padding:12,usePointStyle:true,pointStyleWidth:8}},tooltip:{callbacks:{label:ctx=>` ${ctx.label}: ${formatMoney(ctx.parsed)}`}}}}});
    }
}

function renderLotes(lotes){
    const tbody=document.getElementById('lotes-body');
    if(!lotes?.length){tbody.innerHTML=`<tr><td colspan="7"><div class="empty-state"><div class="empty-state-icon">◈</div><div class="empty-state-text">No hay lotes importados — <a href="/bna/procesar.php" style="color:var(--accent-light)">Importar</a></div></div></td></tr>`;return;}
    tbody.innerHTML=lotes.map(l=>`<tr>
        ...
        <td><a href="/bna/movimientos.php?lote=${l.codigo}" class="btn btn-secondary btn-sm">Ver</a></td>
    </tr>`).join('');
}
...
document.getElementById('btn-confirmar-limpiar').addEventListener('click',async()=>{
    ...
    const res=await fetch('/bna/api/padrones.php?action=resetear_movimientos',{method:'POST'});
    const data=await res.json();
    closeModal('modal-limpiar');btn.disabled=false;btn.textContent='Sí, eliminar todo';
