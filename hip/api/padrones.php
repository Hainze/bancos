...
try {
    switch ($action) {
        ...
        case 'guardar_categoria':
            ...
            $id=(int)($d['id']??0);$n=trim($d['nombre']??'');$c=trim($d['codigo']??'');$t=$d['tipo']??'ambos';
        ...
        case 'listar_movimientos':
            ...
            $st=$pdo->prepare("SELECT COUNT(*),SUM(CASE WHEN tipo='ingreso' THEN importe ELSE 0 END),SUM(CASE WHEN tipo='gasto' THEN ABS(importe) ELSE 0 END) FROM hip_movimientos $w");$st->execute($p);
            [$tot,$ing,$gas]=$st->fetch(PDO::FETCH_NUM);
            $st=$pdo->prepare("SELECT * FROM hip_movimientos $w ORDER BY fecha DESC,id DESC LIMIT $lim OFFSET $off");$st->execute($p);
        ...
        case 'stats_dashboard':
            ...
            $t=$pdo->prepare("SELECT COUNT(*) as total_mov,SUM(CASE WHEN tipo='ingreso' THEN importe ELSE 0 END) as total_ing,SUM(CASE WHEN tipo='gasto' THEN ABS(importe) ELSE 0 END) as total_gasto,SUM(CASE WHEN tipo='ingreso' THEN 1 ELSE 0 END) as cant_ing,SUM(CASE WHEN tipo='gasto' THEN 1 ELSE 0 END) as cant_gasto,SUM(CASE WHEN categoria_id IS NULL THEN 1 ELSE 0 END) as sin_clasificar FROM hip_movimientos WHERE fecha BETWEEN ? AND ?");$t->execute($p);
            $c=$pdo->prepare("SELECT COALESCE(categoria_nombre,'Sin clasificar') as cat,tipo,SUM(ABS(importe)) as total,COUNT(*) as cant FROM hip_movimientos WHERE fecha BETWEEN ? AND ? GROUP BY categoria_nombre,tipo ORDER BY total DESC");$c->execute($p);
            $l=$pdo->query("SELECT * FROM hip_lotes ORDER BY created_at DESC LIMIT 10")->fetchAll();
            echo json_encode(['totales'=>$t->fetch(),'por_categoria'=>$c->fetchAll(),'lotes_recientes'=>$l]); break;

        case 'resetear_movimientos':
            $pdo->exec("DELETE FROM hip_movimientos");$pdo->exec("DELETE FROM hip_lotes");
            echo json_encode(['success'=>true]); break;
