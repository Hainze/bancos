...
header('Content-Disposition: attachment; filename="hip_'.$tipo.'_'.date('Ymd_His').'.csv"');
...
if($tipo==='totales'){
    ...
    $w='WHERE 1=1';$p=[];
    if($desde){$w.=" AND fecha >= ?";$p[]=$desde;}if($hasta){$w.=" AND fecha <= ?";$p[]=$hasta;}
    $st=$pdo->prepare("SELECT COALESCE(categoria_nombre,'Sin clasificar') as categoria,COALESCE(codigo,'—') as codigo,tipo,SUM(CASE WHEN importe>0 THEN importe ELSE 0 END) as total_ing,SUM(CASE WHEN importe<0 THEN ABS(importe) ELSE 0 END) as total_gasto,SUM(importe) as neto,COUNT(*) as cantidad FROM hip_movimientos $w GROUP BY categoria_nombre,codigo,tipo ORDER BY categoria_nombre,tipo");
    $st->execute($p);
    ...
    $w='WHERE 1=1';$p=[];
    if($desde){$w.=" AND fecha >= ?";$p[]=$desde;}if($hasta){$w.=" AND fecha <= ?";$p[]=$hasta;}if($lote){$w.=" AND lote_importacion = ?";$p[]=$lote;}
    $st=$pdo->prepare("SELECT fecha,descripcion,importe,tipo,categoria_nombre,codigo,cuit,dni,numero_cuenta,nombre_cliente,lote_importacion FROM hip_movimientos $w ORDER BY fecha ASC,id ASC");
    $st->execute($p);
