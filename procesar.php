    ...
    <div>
        <div class="card mb-24">
            ...
            <div class="alert alert-info">
                <span>ℹ</span>
                <div>El archivo debe tener exactamente 3 columnas: <strong>Fecha</strong>, <strong>Descripción Extendida</strong> e <strong>Importe</strong>. Formatos aceptados: .xlsx, .xls</div>
            </div>

            <div class="upload-zone" id="upload-zone">
                <input type="file" id="file-input" accept=".xlsx,.xls,.csv">
                <div class="upload-icon">📊</div>
                <div class="upload-title">Arrastrar el Excel aquí</div>
...
document.getElementById('btn-procesar').addEventListener('click', async () => {
    ...
    try {
        const res  = await fetch('/api/procesar_excel.php', { method: 'POST', body: form });
...
function renderPreview(rows) {
    ...
    tbody.innerHTML = rows.map(r => `
        <tr>
            ...
            <td class="mono ${r.tipo === 'ingreso' ? 'green' : 'red'}" style="color:${r.tipo==='ingreso'?'var(--green)':'var(--red)'}">${formatMoney(r.importe)}</td>
...
document.getElementById('btn-descargar').addEventListener('click', () => {
    ...
    window.location.href = `/api/exportar_excel.php?tipo=movimientos&lote=${encodeURIComponent(lote)}`;
});
</script>
