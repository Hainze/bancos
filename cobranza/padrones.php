...
<div class="flex-between mb-16">
    <div>
        <p style="color:var(--sub);font-size:14px;margin:0">
            Asigná observaciones a clientes específicos: quién los llama, si no deben llamarse, notas especiales, etc.
            Estas observaciones se agregan automáticamente al exportar el Excel.
        </p>
    </div>
    <button class="btn btn-primary" onclick="abrirModal()">+ Nueva asignación</button>
</div>
...
<div class="modal-overlay" id="modal-padron">
    <div class="modal">
        <div class="modal-header">
            <span class="modal-title" id="modal-title">Nueva asignación</span>
        ...
        <div class="grid-2">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="padron-codigo" placeholder="Ej: 2430">
            </div>
        </div>

        <div class="form-group">
            <label class="form-label">Observación *</label>
            <input type="text" class="form-control" id="padron-obs" placeholder="Ej: Llama Ulises · No llamar más · Cliente VIP">
            <small style="color:var(--sub);font-size:11px;margin-top:4px;display:block">
                Ejemplos: "Llama Ulises", "No llamar más", "Paga los 10 del mes", "Número nuevo: 1234-5678"
            </small>
        </div>
...
<div class="modal-overlay" id="modal-import">
    <div class="modal" style="width:540px">
        <div class="modal-header">
            <span class="modal-title">Importar padrones desde Excel</span>
            <button class="modal-close">✕</button>
        </div>
        <div class="alert alert-info" style="background:rgba(37,99,235,.08);border:1px solid rgba(37,99,235,.3);border-radius:8px;padding:12px 16px;font-size:13px;margin-bottom:16px">
            <div>El Excel debe tener estas columnas: <strong>sistema · codigo · observacion</strong><br>
            La primera fila es el encabezado. Si el cliente ya existe, se actualiza la observación.</div>
        </div>
...
async function loadPadrones(page = 1) {
    ...
    const res      = await fetch('/cobranza/api/padrones.php?' + params);
    ...
    if (!data.data?.length) {
        tbody.innerHTML = `<tr><td colspan="5"><div class="empty-state">
            ...
            <div class="empty-state-text">No hay asignaciones. Creá la primera.</div>
        </div></td></tr>`;
...
async function guardarPadron() {
    ...
    const res  = await fetch('/cobranza/api/padrones.php?action=guardar', {
...
async function eliminarPadron(id) {
    confirmAction('¿Eliminar esta asignación?', async () => {
        const res  = await fetch('/cobranza/api/padrones.php?action=eliminar', {
...
initDropZone('upload-import', async file => {
    ...
    reader.onload = e => {
        ...
        if (iSis < 0 || iCod < 0 || iObs < 0) {
            toast('No se encontraron las columnas sistema, codigo, observacion', 'error'); return;
        }
...
async function confirmarImport() {
    ...
    const res  = await fetch('/cobranza/api/padrones.php?action=importar', {
