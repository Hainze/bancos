...
<div class="filter-bar">
    ...
    <div class="filter-bar-body">
        ...
        <div class="filter-actions">
            ...
            <a href="/bancos/procesar.php" class="btn btn-success">⬆ Importar</a>
...
<div class="card">
    <div class="flex-between mb-16">
        ...
        <a href="/bancos/movimientos.php" class="btn btn-secondary btn-sm">≡ Ver todos</a>
    </div>
...
function renderLotes(lotes) {
    ...
    if (!lotes?.length) {
        tbody.innerHTML = `<tr><td colspan="7"><div class="empty-state">
            ...
            <div class="empty-state-text">No hay lotes importados — <a href="/bancos/procesar.php" style="color:var(--accent-light)">Importar</a></div>
        </div></td></tr>`;
        return;
    }
    tbody.innerHTML = lotes.map(l => `<tr>
        ...
        <td><a href="/bancos/movimientos.php?lote=${l.codigo}" class="btn btn-secondary btn-sm">Ver</a></td>
    </tr>`).join('');
}
...
document.getElementById('btn-confirmar-limpiar').addEventListener('click', async () => {
    ...
    try {
        const res  = await fetch('/api/padrones.php?action=resetear_movimientos', { method: 'POST' });
        const data = await res.json();
        closeModal('modal-limpiar');
        if (data.success) {
            toast('Datos de prueba eliminados', 'success');
            loadDashboard();
        } else {
