...
if (!isset($_FILES['excel']) || $_FILES['excel']['error'] !== UPLOAD_ERR_OK) { echo json_encode(['error' => 'No se recibió archivo válido']); exit; }

$ext = strtolower(pathinfo($_FILES['excel']['name'], PATHINFO_EXTENSION));
if (!in_array($ext, ['xlsx','xls','csv'])) { echo json_encode(['error' => 'Solo se permiten .xlsx, .xls o .csv']); exit; }

$tmpPath = sys_get_temp_dir() . '/' . uniqid('coop_') . '.' . $ext;
...
try {
    ...
    $rawRows = parseExcelCOOP($tmpPath, $ext);
    ...
    $processed = []; $clasificadas = 0;

    foreach (array_slice($rawRows, 1) as $row) {
        $fechaRaw    = $row[$colFecha] ?? '';
        $descripcion = trim($row[$colDesc] ?? '');
        if ($formatoDH) {
            $debe=parseNumCOOP($row[$colDebe]??''); $haber=parseNumCOOP($row[$colHaber]??'');
        ...
        $fecha=parseDateCOOP($fechaRaw); $tipo=$importe>=0?'ingreso':'gasto';
        ...
        foreach ($keywordMap as $kw) {
            ...
            if (strpos($descLower,$kw['palabra'])!==false){$cat_id=$kw['id'];$cat_nom=$kw['nombre'];$cat_cod=$kw['codigo'];break;}
        }
    ...
    foreach ($processed as $p) $stmt->execute([':fecha'=>$p['fecha']?:date('Y-m-d'),':desc'=>$p['descripcion'],':imp'=>$p['importe'],':tipo'=>$p['tipo'],':cat_id'=>$p['categoria_id'],':cat_nom'=>$p['categoria'],':cod'=>$p['codigo'],':cuit'=>$p['cuit'],':dni'=>$p['dni'],':cuenta'=>$p['numero_cuenta'],':nombre'=>$p['nombre'],':lote'=>$p['lote']]);
    ...
    @unlink($tmpPath);
    echo json_encode(['success'=>true,'lote'=>$loteCode,'total'=>count($processed),'clasificadas'=>$clasificadas,'sin_clasificar'=>$sinClas,'formato'=>$formatoDH?'Debe/Haber':'Importe firmado','rows'=>$processed]);
} catch (Exception $e) { @unlink($tmpPath); echo json_encode(['error'=>$e->getMessage()]); }

function parseExcelCOOP($path,$ext){$rows=[];if($ext==='csv'){if(($h=fopen($path,'r'))!==false){$f=fgets($h);rewind($h);$d=(substr_count($f,';')>=substr_count($f,','))?';':',';while(($r=fgetcsv($h,0,$d))!==false)$rows[]=$r;fclose($h);}}else $rows=readXlsxCOOP($path);return $rows;}
function readXlsxCOOP($path){$rows=[];$zip=new ZipArchive();if($zip->open($path)!==true)return $rows;$ss=[];$sx=$zip->getFromName('xl/sharedStrings.xml');if($sx){$xml=simplexml_load_string($sx);if($xml)foreach($xml->si as $si){if(isset($si->t))$ss[]=(string)$si->t;else{$s='';foreach($si->r as $r)$s.=(string)$r->t;$ss[]=$s;}}}$shX=$zip->getFromName('xl/worksheets/sheet1.xml');$zip->close();if(!$shX)return $rows;$xml=simplexml_load_string($shX);if(!$xml)return $rows;foreach($xml->sheetData->row as $row){$rd=[];foreach($row->c as $cell){$ref=(string)$cell['r'];preg_match('/^([A-Z]+)(\d+)$/',$ref,$m);$ci=colCOOP($m[1]);while(count($rd)<$ci)$rd[]='';$t=(string)$cell['t'];$v=(string)$cell->v;if($t==='s')$v=$ss[(int)$v]??'';elseif(isset($cell->v)&&$ci===0&&is_numeric($v)&&(float)$v>40000)$v=dateCOOP((float)$v);$rd[]=$v;}$f=array_filter($rd,fn($x)=>$x!==''&&$x!==null);if(!empty($f))$rows[]=$rd;}return $rows;}
function colCOOP($l){$i=0;foreach(str_split(strtoupper($l))as $c)$i=$i*26+(ord($c)-64);return $i-1;}
function dateCOOP($s){return date('Y-m-d',(int)(($s-25569)*86400));}
function parseDateCOOP($v){if(empty($v))return null;$v=trim($v);if(preg_match('/^\d{4}-\d{2}-\d{2}$/',$v))return $v;if(preg_match('/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/',$v,$m))return sprintf('%04d-%02d-%02d',$m[3],$m[2],$m[1]);if(preg_match('/^(\d{1,2})-(\d{1,2})-(\d{4})$/',$v,$m))return sprintf('%04d-%02d-%02d',$m[3],$m[2],$m[1]);$t=strtotime($v);return $t?date('Y-m-d',$t):date('Y-m-d');}
function parseNumCOOP($v){if(is_numeric($v))return(float)$v;$v=str_replace(['$','  ',"\xc2\xa0"],['','',''],trim($v));if(preg_match('/^-?[\d\.]+,\d{1,2}$/',$v)){$v=str_replace('.','',$v);$v=str_replace(',','.',$v);}else $v=str_replace([',',' '],['',''],$v);return(float)$v;}
function normalizeStrCOOP($s){return iconv('UTF-8','ASCII//TRANSLIT',mb_strtolower($s))?:mb_strtolower($s);}
?>
